...
一个 1 秒 Tick 分别启动 Trenches、1m Rank、5m Rank；每个来源最多一个在途请求，慢来源只跳过自身 Tick。基础请求权重为 3+1+1=5/s，本地令牌桶上限为 10 weight/s；剩余预算供按需 Security，离线 Kline/Pool 优先级最低。
...
选择理由：统一调度保留高频数据又无需三个周期配置。替代方案为每个接口设置独立 Cron 会让重叠、预算和调参更难理解。
...
- [1 秒轮询加候选 Security 可能触发限流] → 全局 weight 10/s、本地令牌桶、候选预热边界、离线任务降级和按 reset 时间全局冷却。
