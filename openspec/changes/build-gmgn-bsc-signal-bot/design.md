...
Outcome Worker 使用持久化到期作业，在 T+15m、T+1h 拉取从 sent_at 开始的 30 秒 Kline。每个检查点在到期后 10 分钟内最多重试 3 次并写入终态。Kline 成功但为空时，优先使用同一观察窗口内已保存的 1m Rank 非退出价格快照，只有 1m Rank 无价格时才使用 5m Rank；快照兜底只计算实际观察到的 MFE、MAE、倍数触达和首次 2x 时间，固定时点收益仅在目标时刻前 10 秒内存在新鲜价格时填写。撤池仍仅在已有 Pool 基线和二次成功复查证据完整时成立。

质量统计以完整 T+1h 结果为统一分母，按观察期间最高价相对 `sent_price` 是否达到 1.2x、1.5x、2x、3x、5x 汇总；代币在 15 分钟前停止交易也不会被排除，`no_trade` 和已确认 `pool_removed` 均作为未命中保留在分母。已毕业信号在 T+1h 即使仍有历史 Kline，也低频复查一次发送时记录的 Pool；这样同一信号可以同时保留此前的倍数触达并标记后来确认的撤池。T+15m 与 1m/5m/15m/1h 固定时点收益仅作为详细诊断。达到 2x 的用时取首次实时价格触达时刻，或首次触达该价格的 30 秒 Kline 结束时刻，因此是保守近似而非成交证明。
...
选择理由：实时路径不应为统计阻塞；明确终态和覆盖率可避免数据缺失导致幸存者偏差。Rank 快照已经由实时主链路持久化，以它作为空 Kline 的有限兜底无需增加 API、服务或配置，同时避免把已观察到持续行情的代币误判为无交易。
...
- [单币 Security 契约缺失造成健康误报警] → 候选始终失败关闭；单次和两次失败标记 degraded，连续三次才判定依赖 failed，任一成功恢复 healthy。
