...
质量统计以完整 T+1h 结果为统一分母，按观察期间最高价相对 `sent_price` 是否达到 1.2x、1.5x、2x、3x、5x 汇总；代币在 15 分钟前停止交易也不会被排除，`no_trade` 和已确认 `pool_removed` 均作为未命中保留在分母。T+15m 与 1m/5m/15m/1h 固定时点收益仅作为详细诊断。达到 2x 的用时取首次实时价格触达时刻，或首次触达该价格的 30 秒 Kline 结束时刻，因此是保守近似而非成交证明。
