...
- 记录信号后 15m/1h 结果，以 1 小时观察窗口内达到 1.2x/1.5x/2x/3x/5x 的倍数命中率为主要质量指标，并保留固定时点收益、MFE/MAE、覆盖率和延迟作为诊断。
- 将变化快照、Security 结果、配置版本和信号结果保存到 SQLite；额外以每分钟最多 5 个的固定上限保存 Security 已通过预热候选的紧凑研究样本和结果，用于阈值回算。
- 加入 Schema 失败关闭、429 全局冷却、请求超时、持久化去重、Telegram 模糊发送保护、数据陈旧和存储异常降级。私人开发频道立即接收按正式规则通过的信号；正式频道以有效信号、各路径、请求和延迟样本门槛验收，不再以固定运行时长代替质量样本。
