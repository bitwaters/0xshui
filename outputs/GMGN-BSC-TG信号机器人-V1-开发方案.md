...
满足预热条件后立即异步查询 Token Security，同时继续收集动量快照。Security 成功结果可缓存 60 秒用于候选预筛，但正式发送时检查结果年龄不得超过 10 秒；超过 10 秒必须刷新。
...
前两项是 Bot 目标，不承诺链上事件到 GMGN 可见的时间。72 小时影子运行后用实测值替换估算；若 P95 不达标，先检查 Security 队列和 GMGN 响应时间，不通过缩短轮询间隔掩盖上游延迟。
...
T+15m 拉取一次从发送时刻开始的 30 秒 Kline，计算 1m/5m/15m 收益与 15m MFE/MAE；T+1h 再拉取一次补齐 1h 收益与 1h MFE/MAE。这样既能在 15 分钟及时生成命中率，又不需要为每条信号发起四次结果请求。
...
不能仅凭 Kline 为空判断 `pool_removed`。只有系统曾成功记录过该代币的有效 DEX 池，随后 Kline 为空，再调用一次低频 `token pool` 且成功确认原池缺失或流动性为 0，同时没有可用替代池，才能标记 `pool_removed`。尚未毕业的曲线币没有 DEX 池属于正常状态，不能据此判撤池；Pool 请求报错时也只能标记 `api_missing/retry_exhausted`。该回退不进入实时路径。
...
```text
15分钟命中：信号后15分钟内最高涨幅 >= 30%
1小时大涨：信号后1小时内最高涨幅 >= 100%
```

只统计已经完成对应观察周期的信号。尚未满 15 分钟或 1 小时的信号显示为“待评估”，不计入失败。
...
命中率和 MFE 使用区间最高价，是“价格曾触达”的研究指标，不代表用户能以该价格成交；统计卡片必须保留这一说明。
...
默认 `/stats` 使用配置的报告时区统计今日数据，数据库内部统一使用 UTC。
...
每类显示信号数、命中率、中位最高涨幅、中位最大回撤和平均延迟。详细卡片用于调参，不主动每日推送。
...
内存实时检测使用每次成功返回的数据，不受持久化降采样影响。默认快照保留 14 天，`signals`、`signal_outcomes` 和配置版本保留 180 天。SQLite 设置 5 GB 软上限，超过后优先清理最旧普通榜单快照，再清理过期高频快照，不删除信号及结果。数据库初始化启用 WAL 和 `auto_vacuum=INCREMENTAL`；每日清理后执行 WAL checkpoint 与增量回收，避免“删了记录但文件仍持续膨胀”。
...
- 完整拉取和计算。
- 保存所有候选、过滤原因、模拟信号和后续结果。
- 不向正式频道发送，或只发送到私有测试频道。
...
- 全局限频与创建者限频可回算验证。
- `/stats` 不把待评估信号计入失败。
- `no_trade/pool_removed` 不会从命中率分母消失；前者收益为 0%，后者保守为 -100%；`api_missing` 会降低并展示数据覆盖率。
- 相同快照和配置版本回算结果完全一致。
...
72 小时样本不足以证明长期收益，只用于发现明显问题。至少观察：
