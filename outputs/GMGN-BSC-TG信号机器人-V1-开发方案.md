...
若 Kline 请求成功但返回空数据，优先使用同一观察窗口内已保存的 `rank_1m` 非退出价格快照；只有 `rank_1m` 没有有效价格时才使用 `rank_5m`。快照兜底只统计实际观察到的最高/最低倍数和首次 2x 触达时间；固定时点收益仅在目标时刻前 10 秒内存在新鲜价格时填写，不能拿早已离榜前的旧价格冒充 T+15m 或 T+1h。两种 Rank 都没有价格时，才继续使用下述 Pool/Trenches 证据判定终态。
...
- 全局限频与创建者限频可回算验证。
- `/stats` 不把待评估信号计入失败。
- 空 Kline 有 `rank_1m` 快照时按 1m Rank 完成结果，只有 1m 缺失时才回退到 `rank_5m`；退出快照和陈旧固定时点价格不会被误用。
